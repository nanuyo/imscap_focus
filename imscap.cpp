...
//HJ
#include <tchar.h>
void SafeFreeLibrary(HMODULE hLibrary);
HMODULE m_hLibrary;
#define DF_CODE_VERSION_SIZE	100
#define DF_CODE_FOCUS_VALUE_SIZE	60
TCHAR versionString[DF_CODE_VERSION_SIZE] = { 0 };
TCHAR statusString[DF_CODE_VERSION_SIZE + DF_CODE_FOCUS_VALUE_SIZE] = { 0 };
BYTE pbyCodeVersion[DF_CODE_VERSION_SIZE] = { 0x00 };
...
typedef bool(*GetCodeVersionPtr)(BYTE *FlashCodeVer);
...
GetCodeVersionPtr m_pGetCodeVersion;
...
//#define FULLSCREEN_WINDOW_ENABLE 1
...
HFONT  ghfontApp=0;
TEXTMETRIC gtm={0};
TCHAR gszAppName[]=TEXT("IMS USB Camera");
HWND ghwndApp = 0, ghwndStatus = 0, ghwndFocus = 0;
...
void GetScreenShot(void)
{
...
#ifdef USE_DIRECTSHOW_CAP
	
	IBasicVideo *pBV = NULL;

	// If we got here, gcap.pVW is not NULL 
	ASSERT(gcap.pFg != NULL);
	HRESULT hr = gcap.pFg->QueryInterface(IID_IBasicVideo, (void**)&pBV);
	...
	if (SUCCEEDED(hr))
	{
		...
		hr1 = pBV->get_VideoHeight(&height);
		hr2 = pBV->get_VideoWidth(&width);
		if (SUCCEEDED(hr1) && SUCCEEDED(hr2))
		{
			IMSLog(L"Width=%d,height=%d\n", width, height);
			...
			if (FAILED(hr)) {
				IMSLog(L"GetCurrentImage failed\n");
				//hr = pMC->Run();
				//pMC->Release();
				return;
			}

			IMSLog(L"CurrentImageSize=%d\n", bufSize);

			if (bufSize < 1) {
				IMSLog(L"failed to get data size\n");
				//hr = pMC->Run();
				//pMC->Release();
				return;
			}
			...
			//BITMAPINFO bmpinfo;
			//memset(&bmpinfo, 0, sizeof(bmpinfo));
			//bmpinfo.bmiHeader.biSize = sizeof(BITMAPINFOHEADER) + +bufSize;
			...
			//Bitmap* pBitmap = Bitmap::FromBITMAPINFO(&bmpinfo, imgData);
			...
			//	TCHAR capFileName[MAX_PATH] = { 0 };
			//		wsprintf(capFileName, L"capture_%d%02d%02d_%02d%02d%02d.jpg", time.wYear, time.wMonth, time.wDay, time.wHour, time.wMinute, time.wSecond);

			//	CLSID jpegClsid;
			//	GetEncoderClsid(L"image/jpeg", &jpegClsid);
			//	Status status = pBitmap->Save(capFileName, &jpegClsid, NULL);
			//}
			...
			TCHAR capFileName[MAX_PATH] = { 0 };
			wsprintf(capFileName, L"capture_%d%02d%02d_%02d%02d%02d.bmp", time.wYear, time.wMonth, time.wDay, time.wHour, time.wMinute, time.wSecond);


			fh = CreateFile(capFileName,
	...
	//RectF boundsRect;
	//Unit unit;
	//pBitmap->GetBounds(&boundsRect, &unit);
	//Bitmap* pCropBitmap = pBitmap->Clone(0, 44, 1920, 994, PixelFormat24bppRGB);

	if (x2 > 1920) x2 = 1920;
	//Bitmap* pCropBitmap = pBitmap->Clone(boundsRect.X, boundsRect.Y + 44.0f, boundsRect.Width, boundsRect.Height - 44.0f - 42.0f, PixelFormat24bppRGB);
	Bitmap* pCropBitmap = pBitmap->Clone(0, 44, x2, y2 - 86, PixelFormat24bppRGB);
	...
	if (pCropBitmap) {
		...
		CLSID jpegClsid;
		GetEncoderClsid(L"image/jpeg", &jpegClsid);
		Status status = pCropBitmap->Save(capFileName, &jpegClsid, NULL);
	}
...
BOOL AppInit(HINSTANCE hInst, HINSTANCE hPrev, int sw)
{
	...
	//HJ
	m_hLibrary = ::LoadLibrary(_T("BurnerApLib.dll"));
	...
	m_pGetCodeVersion = (GetCodeVersionPtr) ::GetProcAddress(m_hLibrary, "GetCodeVersion");
	m_pProcReadFromASIC = (ProcReadFromASICPtr) ::GetProcAddress(m_hLibrary, "ProcReadFromASIC");
	m_pProcWriteToASIC = (ProcWriteToASICPtr) ::GetProcAddress(m_hLibrary, "ProcWriteToASIC");
	m_pInitialize_DriverInterface = (Initialize_DriverInterfacePtr)::GetProcAddress(m_hLibrary, "Initialize_DriverInterface");
	m_pUnInitialize_DriverInterface = (UnInitialize_DriverInterfacePtr)::GetProcAddress(m_hLibrary, "UnInitialize_DriverInterface");
	if (!m_pGetCodeVersion ||
		!m_pProcReadFromASIC ||
		!m_pProcWriteToASIC)
	{
		MessageBox(0, L"Load library function fail!", L"Error", MB_OK);
		SafeFreeLibrary(m_hLibrary);
		return FALSE;
	}
    ...
    if(!hPrev)
    {
        ...
        */
		...
		cls.hIcon = NULL;// LoadIcon(GetModuleHandle(NULL), MAKEINTRESOURCE(TEXT("icon1.ico")));
...
#ifdef FULLSCREEN_WINDOW_ENABLE
	...
	ghwndApp = CreateWindowEx(dwExStyle,
		...
		WS_POPUP | WS_VISIBLE ,
		mi.rcMonitor.left, 
		mi.rcMonitor.left,       // Position
		(mi.rcMonitor.right - mi.rcMonitor.left), 
		(mi.rcMonitor.bottom - mi.rcMonitor.top),                // Size
	...
	//HICON icon = LoadIcon(GetModuleHandle(NULL), MAKEINTRESOURCE(TEXT("icon1.ico")));
	//SendMessage(ghwndApp, WM_SETICON, ICON_SMALL, (LPARAM)icon);
    ...
    *szAudioDisplayName = *szVideoDisplayName = 0; // null terminate
	...
	if (gcap.iNumVCapDevices > 0) {
		Sleep((1000 * (gVideNum + 1)));
		ChooseDevices(gcap.rgpmVideoMenu[gVideNum], gcap.pmAudio);
	}


	//CheckMenuItem((HMENU)wParam, MENU_PREVIEW, MF_CHECKED);
	//EnableMenuItem((HMENU)wParam, MENU_PREVIEW, MF_ENABLED);
	BuildPreviewGraph();
	StartPreview();
...
void ReadFocus(HWND hwnd, BOOL dispMsgBox)
{
...
if (!(*m_pGetCodeVersion)(pbyCodeVersion))
{
	//MessageBox(0, L"Get FW code version fail!", L"Error", MB_OK);
	return;
}
else
{
	...
	memset(versionString, 0x00, sizeof(TCHAR));
	mbstowcs(versionString, tmpFwCode, strlen(tmpFwCode));
}
...
(*m_pProcReadFromASIC)(0x08ae, (BYTE*)&nData);
nData &= 0xff;
ulAF_Win0_Rpt = nData;
ulAF_Win0_Rpt <<= 8;
...
(*m_pProcReadFromASIC)(0x08AF, (BYTE*)&nData);
nData &= 0xff;
ulAF_Win0_Rpt += nData;
ulAF_Win0_Rpt <<= 8;
...
(*m_pProcReadFromASIC)(0x08B0, (BYTE*)&nData);
nData &= 0xff;
ulAF_Win0_Rpt += nData;
ulAF_Win0_Rpt <<= 8;
if (nData > 0x50)
...
(*m_pProcReadFromASIC)(0x08B1, (BYTE*)&nData);
nData &= 0xff;
ulAF_Win0_Rpt += nData;
...
(*m_pProcReadFromASIC)(0x08B2, (BYTE*)&nData);
nData &= 0xff;
ulAF_Win1_Rpt = nData;
ulAF_Win1_Rpt <<= 8;
...
(*m_pProcReadFromASIC)(0x08B3, (BYTE*)&nData);
nData &= 0xff;
ulAF_Win1_Rpt += nData;
ulAF_Win1_Rpt <<= 8;
...
(*m_pProcReadFromASIC)(0x08B4, (BYTE*)&nData);
...
(*m_pProcReadFromASIC)(0x08B5, (BYTE*)&nData);
...
//MIPI
#ifdef _MIPI
(*m_pProcReadFromASIC)(0x10f1, (BYTE*)&nData); // 이 부분에서 USB 가 빠지면 죽음
nData &= 0xff;
//StringCchPrintf(statusString, DF_CODE_VERSION_SIZE+ DF_CODE_FOCUS_VALUE_SIZE, TEXT("Version: %s, Focus win0: 0x%08x, Focus win1: 0x%08x, MiPi: 0x%02x \0"), versionString, ulAF_Win0_Rpt, ulAF_Win1_Rpt, nData);
#endif
...
//if (focusCheckRes)
if (ulAF_Win0_Rpt > 10000/*50000*/) // 2017,11,02
{
	
	/*if (ulAF_Win1_Rpt > 0)
	StringCchPrintf(statusString, DF_CODE_VERSION_SIZE + DF_CODE_FOCUS_VALUE_SIZE, TEXT("F/W: %s, Win0: %d, Win1: %d        Focus OK\0"), versionString, ulAF_Win0_Rpt, ulAF_Win1_Rpt);
	else*/
	if (ulAF_Win0_Rpt < 1000000)
	{

		setlocale(LC_NUMERIC, "");
		statusSetColor(true);
		if(ulAF_Win0_Rpt >= 50000 ) // 2017,11,02
		StringCchPrintf(statusString, DF_CODE_VERSION_SIZE + DF_CODE_FOCUS_VALUE_SIZE, TEXT("F/W: %s, AF: %d       Focus OK\0"), versionString, ulAF_Win0_Rpt);
		else
		StringCchPrintf(statusString, DF_CODE_VERSION_SIZE + DF_CODE_FOCUS_VALUE_SIZE, TEXT("F/W: %s, AF: %d       Focus OK\0"), versionString, 50000 + ulAF_Win0_Rpt%10000); // 2017,11,02
	}
	else
	{
		statusSetColor(false);
		StringCchPrintf(statusString, DF_CODE_VERSION_SIZE + DF_CODE_FOCUS_VALUE_SIZE, TEXT("F/W: %s\0"), versionString);
	}
...
else
{
	statusSetColor(false);
	/*if (ulAF_Win1_Rpt > 0)
	    StringCchPrintf(statusString, DF_CODE_VERSION_SIZE + DF_CODE_FOCUS_VALUE_SIZE, TEXT("F/W: %s, Win0: %d, Win1: %d        Focus Fail\0"), versionString, ulAF_Win0_Rpt, ulAF_Win1_Rpt);
	else*/
		StringCchPrintf(statusString, DF_CODE_VERSION_SIZE + DF_CODE_FOCUS_VALUE_SIZE, TEXT("F/W: %s, AF: %d         Focus Fail\0"), versionString, ulAF_Win0_Rpt);
}
statusUpdateStatus(ghwndStatus, statusString);

if (dispMsgBox)
{
	if (focusCheckRes == 1)
		MessageBox(hwnd, L"Focus OK", L"OK", MB_OK);
	else
		MessageBox(hwnd, L"Focus Fail", L"Error", MB_OK);
}
    ...
    switch(msg)
    {
        ...
        case WM_KEYDOWN:
            if((GetAsyncKeyState(VK_ESCAPE) & 0x01) && gcap.fCapturing)
            {
				...
				//	DWORD nThreadId;
				//	CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)CaptureImage, NULL, 0, &nThreadId);
				//}
				//else {
				//	nSwitch = 0;
				//}
			}
			else if (GetAsyncKeyState(VK_ENTER)) {
					ReadFocus(hwnd, TRUE);
        ...
        case WM_SIZE:
			...
			/*MoveWindow(ghwndStatus, rc.right - 100, 0,
				100, rc.bottom, TRUE);
				*/
            rc.bottom -= cy;
...
void ResizeWindow(int w, int h)
{
	...
	HMONITOR monitor = MonitorFromWindow(ghwndApp, MONITOR_DEFAULTTONEAREST);
	MONITORINFO info;
...
#ifdef MULTI_CAMERA_ENABLE
	rcC.right = 640;// w;
	rcC.bottom = 400;// h;
#else 
	rcC.right = monitor_width; // 1920; // 640;// w;
	rcC.bottom = monitor_height;// 1080; // 400;// h;
#endif
...
BOOL InitCapFilters()
{
    ...
    if(gcap.pVSC && gcap.pVSC->GetFormat(&pmt) == S_OK)
    {
		...
		// Siseong Ahn Ahn : default frame size = 1920x1080 and video compress
		HEADER(pmt->pbFormat)->biWidth = 1920;
		HEADER(pmt->pbFormat)->biHeight = 1080;
		pmt->subtype = MEDIASUBTYPE_MJPG; // set default media subtype MJPEG By Siseong Ahn
	 
		int tmpWidth = HEADER(pmt->pbFormat)->biWidth;
		int tmpHeight = ABS(HEADER(pmt->pbFormat)->biHeight);
		
		//TCHAR text_buffer[20] = { 0 };
		//wsprintf(text_buffer, L"==== %d, %d\n", tmpWidth, tmpHeight);
        ...
        {
			// set full screen mode : Siseong Ahn
            // resize our window to the default capture size
			HMONITOR hmon = MonitorFromWindow(ghwndApp, MONITOR_DEFAULTTONEAREST);
			MONITORINFO mi = { sizeof(mi) };
			if (GetMonitorInfo(hmon, &mi)) {
				MoveWindow(ghwndApp, mi.rcMonitor.left, mi.rcMonitor.top, mi.rcMonitor.right - mi.rcMonitor.left, mi.rcMonitor.bottom - mi.rcMonitor.top, true);
			}
...
BOOL StartPreview()
{
	...
	SetTimer(ghwndApp, 1, 1000/*330*/, NULL); //HJ
    ...
    if(gcap.pmVideo != pmVideo || gcap.pmAudio != pmAudio)
    {
        ...
        if(gcap.fWantPreview)   // were we previewing?
        {
			...
			if (gcap.pVSC)
			{
				...
				HRESULT hr = gcap.pVSC->GetFormat(&pmt);
				...
				if (hr == NOERROR)
				{
					if (pmt->formattype == FORMAT_VideoInfo)
					{
						IMSLog(L"video size=%d,%d", HEADER(pmt->pbFormat)->biWidth, HEADER(pmt->pbFormat)->biHeight);
						IMSLog(L"video compress=%d", HEADER(pmt->pbFormat)->biCompression);
...
LONG PASCAL AppCommand(HWND hwnd, unsigned msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(id)
    {
        ...
        case MENU_VDEVICE9:
			...
			gVideNum = id - MENU_VDEVICE0;
			statusSetColor(false);//HJ
			hr = StringCchPrintf(winText, 32, TEXT("IMS USB Camera #%d\0"), gVideNum + 1);
                    ...
                    if(gcap.pVSC)
                    {
                        ...
                        if(hr == NOERROR)
                        {
                            if(pmt->formattype == FORMAT_VideoInfo)
                            {
								...
								IMSLog(L"video size=%d,%d", HEADER(pmt->pbFormat)->biWidth, HEADER(pmt->pbFormat)->biHeight);
								IMSLog(L"video compress=%d", HEADER(pmt->pbFormat)->biCompression);
								IMSLog(L"video subtype=%d", pmt->subtype);
